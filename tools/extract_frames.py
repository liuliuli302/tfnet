        ...
        if success:
            if frame_count % frame_interval == 0:
                frame_filename = os.path.join(
                    output_folder, f"{saved_frame_count:06d}.jpg")
                cv2.imwrite(frame_filename, frame)
                saved_frame_count += frame_interval
            frame_count += 1
        else:
            ...
            if frame_count == 0:  # No frames were read at all
                print(
                    f"Error: No frames could be read from video {video_path}. It might be corrupted or an unsupported format.")
...
if __name__ == '__main__':
    ...
    if os.path.exists(test_video_path):
        print(
            f"Testing single video extraction from '{test_video_path}' to '{test_output_folder}'")
        extract_frames(test_video_path, test_output_folder, frame_interval=30)
    else:
        print(
            f"Test video '{test_video_path}' not found. Skipping single video test.")
    ...
    if os.path.exists(test_video_dir):
        ...
        extract_frames_from_directory(
            test_video_dir, test_batch_output, frame_interval=30)
    else:
        print(
            f"Test video directory '{test_video_dir}' not found. Skipping batch test.")
...
def main():
    ...
    extract_frames_from_directory(
        ...
        output_folder='/root/autodl-tmp/datasets/SumMe/frames',
        frame_interval=15
    )

    extract_frames_from_directory(
        video_dir='/root/autodl-tmp/datasets/TVSum/videos',
        output_folder='/root/autodl-tmp/datasets/TVSum/frames',
        frame_interval=15
    )

...
def select_frames(folder_list, preprocess, resnet18_pretrained):
    for folder in folder_list:
        ...
        img_loader = DataLoader(img_set, batch_size=64,
        ...
        with torch.no_grad():
            ...
            while len(idx_list) > 5:
                ...
                pop_idx_list = torch.where(
                    data < mu-std*(np.exp(1-loop_idx/config.divlam)))[0].detach().cpu().numpy()
        ...
        for objs_act in objs_acts:
            if objs_act['q_uid'] == question['q_uid']:
                ...
                folder_list = glob.glob(
                    f"{config.img_folder}/{question['q_uid']}/")
                out_frames, output_result = select_frames(
                    folder_list, preprocess, resnet18_pretrained)

...
class Solver02:
    ...
    def _scene_caption(self):
        ...
        # 将结果写入最终 JSON 结构
        dataset_configs = [
            ('summe', summe_scene_caption_json, summe_frame_caption_json, summe_scene_caption_json),
            ('tvsum', tvsum_scene_caption_json, tvsum_frame_caption_json, tvsum_scene_caption_json)
        ]
        ...
        for ds_name, final_json, frame_data, json_file in dataset_configs:
            ...
            for video_name, scenes_map in repo_updates.items():
                # 重建完整的 summaries 列表，确保索引对齐
                cps = cps_map[ds_name][video_name]
                if isinstance(cps, np.ndarray):
                    cps = cps.tolist()

                scene_summaries_list = []
                for i in range(len(cps)):
                    # 如果场景有结果则填入，否则填空串 (可能该场景无关键帧)
                    scene_summaries_list.append(scenes_map.get(i, ""))

                final_json[video_name] = {
                    'picks': frame_data[video_name]['picks'],
                    'scene_summaries': scene_summaries_list
                }
